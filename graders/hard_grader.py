...
def grade(state):
    ...
    remaining_malicious = sum(1 for f in state.files if f.is_malicious and f.id not in state.quarantined_files)
    remaining_suspicious = sum(1 for p in state.processes if p.suspicious and p.id not in state.killed_processes)
    total_threats = max(handled_files + remaining_malicious + handled_procs + remaining_suspicious, 1)
