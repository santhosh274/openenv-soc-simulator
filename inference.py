...
# =========================
# ✅ Strict Score Safety Gate (0.0 < score < 1.0)
# =========================
...
# =========================
# System Prompt (Hardened against premature escalation)
# =========================
...
3. Processes marked [SUSPICIOUS] or with weird names (e.g., svchost32, crypter) → kill immediately.
...
def build_user_prompt(observation, step: int, memory: List[Dict]) -> str:
    ...
    if suspicious_count > 1 or malicious_count > 1:
        multi_hint = f"\n⚠️ ALERT: {malicious_count} malicious file(s) and {suspicious_count} suspicious process(es) detected. Handle ALL threats before considering escalation.\n"
...
# =========================
# 🤖 Robust Heuristic Fallback (No LLM required)
# =========================
def fallback_action(observation, memory: List[Dict]) -> Action:
    ...
    # 1. Contain high-entropy files (>7.0) immediately (likely packed/encrypted malware)
    for f in files:
        fid = f.get("id")
        if fid not in acted_ids and f.get("entropy", 0) > 7.0:
            return Action(type="quarantine_file", target_id=fid)

    # 2. Kill suspicious/orphaned processes
    for p in processes:
        pid = p.get("id")
        if pid not in acted_ids:
            name = p.get("name", "").lower()
            parent = p.get("parent")
            suspicious_kw = any(k in name for k in ["crypt", "encrypt", "ransom", "svchost32", "cmd32", "tmp", "miner"])
            if parent is None or suspicious_kw:
                return Action(type="kill_process", target_id=pid)

    # 3. Investigate highest-entropy unacted file
    ...
    # 4. Investigate unacted processes
